...
std::vector<std::pair<size_t, float>> get_pruned_log_probs(
  ...
  if (log_cutoff_prob < 0.0 || cutoff_top_n < cutoff_len) {
    ...
    if (log_cutoff_prob < 0.0) {
      ...
      for (size_t i = 0; i < prob_idx.size(); ++i) {
        cum_prob = log_sum_exp(cum_prob, log_input ? prob_idx[i].second : std::log(prob_idx[i].second) );
        cutoff_len += 1;
  ...
  for (size_t i = 0; i < cutoff_len; ++i) {
    log_prob_idx.push_back(std::pair<int, float>(
        prob_idx[i].first, log_input ? prob_idx[i].second : std::log(prob_idx[i].second + NUM_FLT_MIN)));
  }
  return log_prob_idx;
}
