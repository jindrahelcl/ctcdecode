...
int main(int argc, char* argv[]) {
  ...
  auto res1 = ctc_beam_search_decoder(probs_seq1, vocab_list, beam_size, 1.0, 40, 6);
  auto res2 = ctc_beam_search_decoder(probs_seq2, vocab_list, beam_size, 1.0, 40, 6);

  Scorer lm(0, 0, "test.arpa", vocab_list);
  auto res_lm = ctc_beam_search_decoder(probs_seq2, vocab_list, beam_size, 1.0, 40, 6, 0, &lm);
